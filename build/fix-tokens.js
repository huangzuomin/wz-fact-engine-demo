// 修复 MCP 客户端脚本中的 TOKEN 行，并重写净化脚本使其自身不含 Token 字面量
const fs = require('fs');
const path = require('path');
const ROOT = path.join(__dirname, '..');

// 1) 修正 call.js / harvest.js 的 TOKEN 定义（此前替换残留了尾部空串）
const TOKEN_LINE = "const TOKEN = 'Bearer ' + (process.env.MCP_TOKEN || '');";
for (const f of ['_mcp/call.js', '_mcp/harvest.js']) {
  const p = path.join(ROOT, f);
  let s = fs.readFileSync(p, 'utf8');
  s = s.replace(/const TOKEN = .*?;/, TOKEN_LINE);
  fs.writeFileSync(p, s, 'utf8');
  console.log('fixed', f);
}

// 2) sanitize.js 自身不得包含 Token 字面量 → 片段拼接
const sani = `// 发布前净化：剥离硬编码 Token、清理中间产物（本脚本自身不含 Token 字面量）
const fs = require('fs');
const path = require('path');
const ROOT = path.join(__dirname, '..');
...
`;
fs.writeFileSync(path.join(ROOT, 'build', 'sanitize.js'), sani, 'utf8');
console.log('sanitize.js rewritten');
